...
{
    public class CandyOddOneOutView : MonoBehaviour, ICandyOddOneOutView
    {

        public event Action BackButtonPressed;
        public event Action<CandyType> ChoosedObject;
        ...
        private Candy RightAnswer;
        private Candy ChoosedCandy;
        ...
        private void Awake()
        {
            ...
            {
                Candys[i].transform.localScale = 0.001f * Vector3.one;
                Candys[i].ObjectChoosed += (t) => HandleObjectClick(t);
            }

        }


        private void HandleObjectClick(Candy target)
        {
            ChoosedCandy = target;

            ChoosedObject(target.GetCandyType());
        }

        public IPromise SetupAndShowObjects(List<CandyType> hand)
        {
            ...
            Candys[posIndexLast].Setup(hand[4]);

            RightAnswer = Candys[posIndexLast];
        ...
        public IPromise RemoveObjects()
        {
            ...
            for (int i = 0; i < Candys.Length; i++)
            {
                ...
                if (Candys[j].gameObject.activeSelf) // not neccesary to remove already removed object (after right answer animation)
                    deferred = (Deferred)Timers.Wait(wait)
                                           .Then(Candys[j].DisAppear);
            }
        ...
        public void SetObjectsInteractable(bool state)
        {
            for (int i = 0; i < Candys.Length; i++)
            {
                Candys[i].SetInteractable(state);
            }
        }

        public IPromise ProccessAnswer(CandyType choosed, bool right)
        {
            var sound = (right) ? Sounds.ECommon.AM_Short : Sounds.ECommon.UhUh;
            SoundManager.Play(sound);

            var candy = ChoosedCandy;
            IPromise action;

            if (right) action = candy.AnimateRightChoice();
            else action = candy.AnimateWrongChoice();
