...
namespace KidGame.Lessons.Candy01
{
    public class Plate : MonoBehaviour
    {
        ...
        private ECandyForm CurrentForm;
        ...
        public void SetCurrentQuest(Candy quest)
        {
            CurrentForm = quest.GetForm();

            var questTransform = quest.transform;

            var pos = QuestSlotPosition[(int)CurrentForm].position;
            var rot = QuestSlotPosition[(int)CurrentForm].rotation;
        ...
        public IPromise PutObjectInPlace(Candy candy)
        {
            ...
            { candy.transform.SetAsLastSibling(); }

            var slot = (CurrentSlot == 0) ? Slot01Position[(int)form] : Slot02Position[(int)form];
        ...
        public IPromise Toss() // toDo: Extend for 3 candies
        {
