...
namespace KidGame.Lessons.Jelly04
{
    public class Jelly : CommonDrag
    {
        ...
        public EJellyFeature GetFeature()
        {
            return (EJellyFeature)MatchType;
        }


        public void Setup(EJellyFeature jellyFeature)
        {
            Sprite sprite;

            var enumInt = Convert.ToInt32(jellyFeature);
            var rnd = Random.Range(0, 10);

            var spriteID = enumInt * 10 + rnd;
