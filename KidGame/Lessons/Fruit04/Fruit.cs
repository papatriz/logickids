...
namespace KidGame.Lessons.Fruit04
{
    public class Fruit : CommonDrag
    {
        ...
        private Vector2 BigSize = new Vector2(220, 330);
        private Vector2 SmallSize = new Vector2(170, 250);
        ...
        private void Setup()
        {
            int num = Enum.GetValues(typeof(EFruitType)).Length;
            int smallShift = num / 2;

            var enumInt = Convert.ToInt32(MatchType);

            RectTransform.sizeDelta = (enumInt < smallShift) ? BigSize : SmallSize;

            var spriteInd = enumInt % smallShift;
