...
{
    public class MakeMedalView : MonoBehaviour, IMakeMedalView
    {
        public event Action MedalFilled;
        ...
        public IPromise ShowTutorial()
        {
            ...
            var mainTarget = Medal.GetTargetCollider().transform;
            arrowTargetsList.Add(mainTarget);
            arrowTargetsList.Add(Medal.GetRibbonTransform());

            var handTarget = Gems[0].transform;

            return Tutorial.Show(arrowTargetsList, handTarget, mainTarget, new Vector3(-2f, 0, 0))
        ...
        public IPromise ShowAllGems(List<EGemType> hand)
        {
            ...
            var targetCollider = Medal.GetTargetCollider();
            var targetType = Medal.GetCurrentType();
            ...
            for (int i = 0; i < hand.Count; i++)
            {
                ...
                gem.SetTarget(targetCollider, targetType);
        ...
        private void HandleGemInFrame(int i)
        {
            ...
            Medal.PutGemIntoFrame(Gems[i])
                .Done(MedalFilled);
        }

        public IPromise SetupAndShowMedal(EGemType gemType)
        {
            return Medal.Appear(gemType);
        }

        public IPromise RemoveMedal()
        {
            return Medal.DisAppear();
        }
